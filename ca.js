...
const CAattrs = [{
  ...
  value: 'Minimalist MITM Proxy CA',
}, {
  shortName: 'OU',
  value: 'CA',
}];
...
const ServerAttrs = [{
  ...
  shortName: 'OU',
  value: 'Minimalist MITM Proxy Server Certificate',
}];
...
CA.create = function (caFolder, callback) {
  ...
  async.series([
    ...
    function (callback) {
      FS.exists(path.join(ca.certsFolder, 'ca.pem'), (exists) => {
        if (exists) {
          ca.loadCA(callback);
        } else {
...
CA.prototype.generateServerCertificateKeys = function (hosts, cb) {
  ...
  FS.writeFile(`${this.certsFolder}/${mainHost.replace(/\*/g, '_')}.pem`, certPem, (error) => {
    if (error) console.error(`Failed to save certificate to disk in ${self.certsFolder}`, error);
  });
  FS.writeFile(`${this.keysFolder}/${mainHost.replace(/\*/g, '_')}.key`, keyPrivatePem, (error) => {
    if (error) console.error(`Failed to save private key to disk in ${self.keysFolder}`, error);
  });
  FS.writeFile(`${this.keysFolder}/${mainHost.replace(/\*/g, '_')}.public.key`, keyPublicPem, (error) => {
